...
class SavingsAccountModel {
  int? total;
  List<SavingsOrdersRows>? rows;
  int? code;
  String? msg;

  SavingsAccountModel({this.total, this.rows, this.code, this.msg});

  SavingsAccountModel.fromJson(Map<String, dynamic> json) {
    total = json['total'];
    if (json['rows'] != null) {
      rows = <SavingsOrdersRows>[];
      json['rows'].forEach((v) {
        rows!.add(new SavingsOrdersRows.fromJson(v));
      });
    }
    code = json['code'];
    msg = json['msg'];
...
class SavingsOrdersRows {
  ...
  SavingsOrdersRows.fromJson(Map<String, dynamic> json) {
    stId = json['stId'];
